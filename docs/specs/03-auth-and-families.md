...
- Optional **allowlist** (`AUTH_ALLOWED_EMAILS`) gating who can *sign up* and who can *create* a new family — see §5.
...
2. **`AUTH_ALLOWED_EMAILS` allowlist** (§5) — an opt-in gate so that on your own deployment, only email addresses you've pre-approved can even **sign up**, and separately, only pre-approved emails can *start* a new family. A stranger who finds the URL and lands on `/signup` is refused, in Italian, before an account is ever created. Because joining a family via invite code requires an account first, on a fully closed instance you must add every intended member's email to this list before they can sign up and use their code — the invite code alone is not enough to get in the door.
...
- Server action `registerWithPassword(name, email, password, confirmPassword)`:
  ...
  2. If `AUTH_ALLOWED_EMAILS` is set and the email isn't in it, reject → "Questa istanza non è aperta a nuove registrazioni. Chiedi un invito a chi la gestisce." (see §5).
...
Document `AUTH_ALLOWED_EMAILS` in `.env.example`: "Leave empty for an open instance (anyone can sign up and start a family). Set to a comma-separated list of emails to restrict who can sign up and who can create a *new* family on this deployment. Joining an existing family via invite code isn't gated by this list directly, but it still requires an account — so every invitee's email must also be on this list before they can sign up and use their code."
...
6. With `AUTH_ALLOWED_EMAILS` set to some other address, `registerWithPassword` for an email not on the list gets the Italian "not open to new registrations" refusal and no `users` row is created. The same instance still lets an already-registered, allowlisted user `createFamily` (or refuses it if that user's email also isn't on the list) and lets any already-registered user `joinFamily` with a valid code. With `AUTH_ALLOWED_EMAILS` unset, both sign-up and `createFamily` are unrestricted.
...
both sign-up and family creation (never on joining via invite code — though
joining still requires an account, so a closed instance requires every
invitee's email on the list too), and the sign-up flow — these are
load-bearing requirements, not optional extras. Follow the "Definition of
done" in 00-overview.md §9. Commit in logical steps.
